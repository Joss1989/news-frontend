  ...
  return (
    <>
      <section className="col-span-4 pb-2 pt-5 flex justify-between">
        <h2>Seneste</h2>
        <Link href="/seneste" className="flex items-center gap-x-2 text-[#e89700]">Se mere <span><FaArrowRight /></span></Link>
      </section>
      {nyheder
        .sort((a, b) => a.publishedAt || 0 - b.publishedAt || 0)
        .slice(0, 4)
        .map((item, index) => (
          <article key={index} className="grid col-span-4 sm:col-span-2 md:col-span-1 grid-rows-1 md:grid-rows-2 gap-x-2 h-full">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-[#e89700] md:mt-auto py-2">{item.articleCategory}</p>
            </div>
          </article>
        ))}
    </>
